    ...
    <>
      {models?.available.length === 0 || !models ? (
        ...
        <Heading
          text={{
            size: "text-4xl",
            className: "animate-appear-default-opacity delay-300",
          }}
        >
          Доступные модели:
        </Heading>
        ...
        <>
          <div className="flex justify-center items-center gap-4">
            {Object.values(models.models)
              ...
              .map((model, index) => (
                <div
                  className="bg-(--bg-secondary) rounded-xl space-y-2 p-4 hover:opacity-90 transition-opacity animate-appear-default-opacity"
                  key={index}
                  style={{
